...
\end{center}
\subsubsection{定電流電源}
資料に直流定電流を流すための電源。電流値の設定、電流反転はパネルにあるスイッチで行う。
\subsubsection{デジボル1}
試料に流した電流により電圧端子間に発生した電圧（四端子法による電圧）を測定するための電圧計として用いる。
\subsubsection{デジボル2}
...
\end{equation}
を得る。これにより、四端子抵抗では式(\ref{ll})の近似のもとで正確に試料の抵抗を測れることがわかる。
\begin{figure}[H]
  \centering
  \begin{subfigure}{0.45\linewidth}
    \centering
    \begin{circuitikz}[american currents]
        \draw (-2, 4) to[short] (-1.5, 4)
        to[battery1=$E$] (0, 4)
        to[rmeter=$R_A$, t=A] (1.5, 4)
        to[short] (2, 4)
        to[short, i=$I$] (2, 2)
        to [rmeter=$R_V$, t=V, i>_=$I_2$] (-2, 2)
        to[short] (-2, 4);
        \draw (2, 2)
        to[european resistor=$r_1$, i>^=$I_1$] (2, -1)
        to[european resistor=$R$] (-2, -1)
        to[european resistor=$r_2$] (-2, 2);
    \end{circuitikz}
    \caption{二端子測定の回路図}
  \end{subfigure}
  \hfill
  \begin{subfigure}{0.45\linewidth}
    ...
    \begin{circuitikz}[american currents]
        ...
        to[rmeter=$R_A$, t=A] (1.5, 4)
        to[short, i=$I$] (2, 4)
        to[european resistor=$r_1$] (2, 1.5)
        to[european resistor=$R$, i>_=$I_1$] (-2, 1.5)
        to[european resistor=$r_2$] (-2, 4);
        \draw (2, 1.5)
        to[european resistor=$r_3$, i>^=$I_2$] (2, -1)
        to[rmeter=$R_V$, t=V] (-2, -1)
        to[european resistor=$r_4$] (-2, 1.5);
    \end{circuitikz}
    \caption{四端子測定の回路図}
  \end{subfigure}
  \caption{測定系の等価回路図}
